...
export const getReferencePointer = (bookName:string, chapterVal:number, verseVal:number, books:Book[])=>{
    ...
    const differentBooks = (f_bk:string|number, s_bk:string|number, f_ch:number|string, s_ch:number|string, f_v:unknown, s_v:number|string)=>{
      if((bookName===f_bk && bookName !== s_bk) || (bookName !== f_bk && bookName===s_bk)){
        ...
        return distancedBook(f_bk, s_bk) 
      }
    }
    ...
    const distancedBook = (f_bk:string|number, s_bk:string|number)=>withinRange(bookAt(f_bk), bookAt(s_bk), bookAt(bookName))
    
    return computedMap.findIndex((ref)=>
