...
import { ArrowDropDown, HourglassBottomRounded, MenuSharp, Note, SearchRounded, Settings, Visibility, VisibilityOff, History, Home, Help, Info, Support, HistoryRounded, HideSource } from '@mui/icons-material'
...
export async function handleAddToTabs(verseAddress: verseAddress) {
  const openedTab = new OpenedTab();

  const {language, versionAbbrev, book_ID, chapter_ID, bookName, verse_ID} = verseAddress;
  
  // const selectedBibleParams: addOpenedTab = { tabID: generateRandomKey(), bookName: bookName, versionAbbrev: versionAbbrev, language: language, book_ID: book_ID, chapter_ID: chapter_ID };

  console.log("Attempted adding to tab", language, versionAbbrev, bookName??"", book_ID, chapter_ID, verse_ID);
...
export const Tab: FC<resolvedOpenedTab> = ({ tabID, book_ID, chapter_ID, language, versionAbbrev, bookName, books }) => {
  ...
  const getTabModelFromRef = (verseRef: string[]) =>{
    let bookNames = formatedBookNames(books)
    let bookId = bookNames.indexOf(verseRef[0])
    let tab:verseAddress = {
      book_ID: bookId,
      chapter_ID: parseInt(verseRef[1]),
      verse_ID: parseInt(verseRef[2]),
      language: language,
      versionAbbrev: versionAbbrev,
      bookName: books[bookId].name
    };
    return tab;
    //  books[bookId].chapters[Number(verseRef[1]) - 1][Number(verseRef[2]) - 1];
  }
                            ...
                            <Box>
                              <Box sx={{ textAlign: "center" }}>
                                {referenceVerse[2] === (id + 1).toString() && referenceVerse[1] === (chapter_ID + 1).toString() && referenceVerse[0] === book.name
                                  ...
                                  :
                                  <Button size='small' onClick={() => { setReferenceVerse([book.name, (chapter_ID + 1).toString(), (id + 1).toString()]) }} color="primary">
                                    <Box sx={{ display: "flex", alignItems: "center" }}>
                                      <Visibility />
                                      <Box sx={{ marginLeft: .8 }}>
                                        See References
                                ...
                                referencesForVerse()?.map((ref: any[], id: React.Key | null | undefined) =>
                                  <Box display="inline-flex" key={id}>
                                    <HtmlTooltip arrow sx={{ fontSize: "35px" }} placement='bottom'
                                      ...
                                      <Button size='small' sx={{ paddingTop: .6 }} onClick={()=>handleAddToTabs(getTabModelFromRef(ref))} variant='text'>{ref.join(" ")}</Button>
                                    </HtmlTooltip>
                                  </Box>)
                                :
                                null
...
export const AppMenu = () => {
  const routes = [
    ...
    { pathname: "/histories", name: "History", MenuIcon: HistoryRounded, type: "link" },
